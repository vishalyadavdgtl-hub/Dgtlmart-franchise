...
export default function FranchiseManagement() {
  ...
  const fetchFranchises = async (showFullLoading = true) => {
    ...
    try {
      ...
      showToast("Failed to load Franchises", "error");
    } finally {
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      // If modal is open for this partner, update it too
      if (selectedPartner && selectedPartner._id === id) {
        setSelectedPartner((prev) => ({ ...prev, status: newStatus }));
      }

      // Refresh data in background without showing full spinner
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      ...
      setFranchises((prev) => prev.filter((p) => p._id !== id));
      if (selectedPartner && selectedPartner._id === id) {
  ...
  const handleCommissionUpdate = async () => {
    ...
    setUpdating(true);
    try {
      await adminAPI.updateFranchiseCommission(selectedPartner._id, {
  ...
  try {
    await adminAPI.updateFranchise(selectedPartner._id, selectedPartner);
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              <div className="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 flex items-center gap-2">
                <span className="text-sm font-bold">
                  Manage franchise partners and approvals
                </span>
              </div>
            </div>
          ...
          <div className="mt-2">
            {loading ? (
              ...
              <div
                className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-h-[400px] ${updating ? "opacity-50 pointer-events-none" : ""}`}
              >
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        {/* <th className="px-4 py-3 text-left font-semibold text-gray-900 border-b border-gray-200 text-center">
                          Referrals
                        </th> */}
                        <th className="px-4 py-3 text-left font-semibold text-gray-900 border-b border-gray-200">
                          Commission
                          ...
                          >
                            ...
                            <td className="px-4 py-4">
                              <div className="flex gap-3">
                                ...
                                <button
                                  onClick={() =>
                                    handleDelete(partner._id, partner.fullName)
                                  }
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Set the total commission amount for{" "}
            <span className="font-semibold">{selectedPartner?.fullName}</span>.
          </p>
          <div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button onClick={handleCommissionUpdate} disabled={updating}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${
                    selectedPartner.partnerType === "franchise"
                      ? "bg-indigo-100 text-indigo-700"
                      : "bg-gray-100 text-gray-700"
                  }`}
                >
                  {selectedPartner.partnerType || "Franchise"} (
                  {selectedPartner.commissionRate || 10}%)
                </span>
              </div>
              <div>
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
              ...
              <div>
                ...
                <p className="text-gray-900 font-medium">
                  {new Date(selectedPartner.createdAt).toLocaleString("en-IN", {
                    dateStyle: "medium",
                    timeStyle: "short",
                  })}
                </p>
            ...
            <div className="border-t border-gray-100 pt-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-sm text-blue-600 mb-1">Total Franchises</p>
                  <p className="text-2xl font-bold text-blue-900">
                    {selectedPartner.FranchiseCount}
                  </p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-900">
                    ₹{selectedPartner.totalCommission.toLocaleString("en-IN")}
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex gap-2 relative">
                      ...
                      {["PENDING", "ACTIVE", "REJECTED"].map((status) => (
                        <button
                          ...
                          disabled={updating}
                          onClick={() =>
                            handleStatusChange(selectedPartner._id, status)
                          }
                ...
                <div className="mt-4 flex gap-3">
                  {selectedPartner.status === "PENDING" && (
                    <button
                      disabled={updating}
                      onClick={() =>
                        handleStatusChange(selectedPartner._id, "ACTIVE")
                      }
                  ...
                  {selectedPartner.status !== "REJECTED" && (
                    <button
                      disabled={updating}
                      onClick={() =>
                        handleStatusChange(selectedPartner._id, "REJECTED")
                      }
